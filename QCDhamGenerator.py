...
from qcd_int_parallel_loc import Basis, NonAbelianHamiltonian
...
def main():
    ...
    parser.add_argument("--Nc", type=int, required=True, help="Number of Colors")
    parser.add_argument("--B", type=int, default=0, help="Charge sector")
    ...
    print(f"Number of Colors: {args.Nc}")
    print(f"Baryon Number: {args.B}")
    print(f"Output file name: {args.out}")
    actual_cores = cpu_count() if args.n_cores == -1 else args.n_cores

    print(f"Number of cores to run on: {actual_cores}")

    # Generate basis states
    print("Generating basis of states...")
    basis = Basis(Emax=args.Emax, L=args.L, M=0, Nc=args.Nc, B=args.B)
    print("Total number of states is: " + str(int(basis.size)))

    # Initialize Hamiltonian
    H = NonAbelianHamiltonian(Emax=args.Emax, L=args.L, M=0, Nc = args.Nc, B=args.B, n_cores=args.n_cores, basis=basis)
